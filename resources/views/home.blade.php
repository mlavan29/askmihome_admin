            ...
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-header no-border d-flex justify-content-between">
                        <h3 class="card-title">{{trans('lang.stores_plural')}}</h3>
...
<script>
    ...
    $(document).ready(function () {
        ...
        ref.orderBy('reviewsCount', 'desc').get().then(async (snapshots) => {
            var html = '';
    ...
    async function getTotalEarnings() {
        ...
        await db.collection('restaurant_orders').where('status', 'in', ["Order Completed"]).get().then(async function (orderSnapshots) {
            ...
            paymentData.forEach((order) => {
                ...
                if (orderData.adminCommission != undefined && orderData.adminCommissionType != undefined && orderData.adminCommission > 0 && total_item_price > 0) {
                    var commission = 0;
